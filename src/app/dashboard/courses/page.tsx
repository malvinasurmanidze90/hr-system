...
export const metadata = { title: 'კურსების ბიბლიოთეკა' };
      ...
      <div className="bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 px-6 pt-8 pb-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1.5">კურსების ბიბლიოთეკა</h1>
              <p className="text-indigo-200 text-sm">სასწავლო კურსები, სავალდებულო პროგრამები და გუნდის განვითარება</p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { icon: GraduationCap, label: 'სულ კურსები',    value: totalCount ?? 0,     sub: 'ყველა სტატუსი'       },
              { icon: TrendingUp,    label: 'გამოქვეყნებული',  value: publishedCount ?? 0,  sub: 'ხელმისაწვდომი'      },
              { icon: FileText,      label: 'მონახაზი',         value: draftCount ?? 0,      sub: 'გამოუქვეყნებელი'   },
              { icon: ShieldCheck,   label: 'სავალდებულო',      value: mandatoryCount ?? 0,  sub: 'სავალდებულო პროგ.' },
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        {needsMigration && (
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-sm text-amber-800">
            ...
            <div>
              <p className="font-semibold mb-1">საჭიროა DB მიგრაცია</p>
              <p className="text-xs mb-2 text-amber-700">
                <code className="bg-amber-100 px-1.5 py-0.5 rounded font-mono">is_mandatory</code> სვეტი არ არსებობს.
                გაუშვეთ Supabase SQL Editor-ში:
              </p>
        ...
        {!error && (!courses || courses.length === 0) && (
          <div className="flex flex-col items-center justify-center py-28 text-center bg-white rounded-2xl border border-gray-200 shadow-sm">
            ...
            <h3 className="text-base font-semibold text-gray-900 mb-2">კურსები არ მოიძებნა</h3>
            <p className="text-sm text-gray-500 max-w-sm leading-relaxed">
              {q || status || cat || mandatory
                ? 'ფილტრის პირობებს შეესაბამებელი კურსები ვერ მოიძებნა. სცადეთ სხვა ფილტრი.'
                : 'ჯერ კურსები არ გაქვთ. შექმენით პირველი კურსი ზევიდან.'}
            </p>
          </div>
        )}
  ...
  return (
    <div className="group flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-1.5 flex-wrap mb-3">
          ...
          {isMandatory ? (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-700 border border-rose-100">
              <ShieldCheck size={10} />სავალდებულო
            </span>
          ) : (
        ...
        {/* Title */}
        <Link href={`/dashboard/courses/${course.id}`} className="block mb-2">
        ...
        <p className="text-sm text-gray-500 line-clamp-2 leading-relaxed flex-1">
          {course.description ?? 'აღწერა არ არის.'}
        </p>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {['კურსი', 'კატეგორია', 'ხანგრძლივობა', 'ტიპი', 'სტატუსი', 'მოქმედება'].map(h => (
                ...
                <tr key={course.id} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    {isMandatory
                      ...
                      : <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500">სურვილისამებრ</span>}
                  </td>
                  <td className="px-5 py-4">
                    <StatusBadge status={course.status} />
                  </td>
      ...
      <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/50">
        <p className="text-xs text-gray-400">{courses.length} კურსი</p>
      </div>
    </div>
  );
}
